...
const MobileStickyCTA = ({ lang = "it" }: MobileStickyCTAProps) => {
  const t = getDict(lang);
  const bookHref = localizePath("/prenotaora", lang);
  const waHref = `https://wa.me/393446107071?text=${encodeURIComponent(t.mobileStickyCta.whatsappPrefill)}`;
    ...
    >
      <div className="px-3 py-2.5 grid grid-cols-2 gap-[2px]">
        ...
        >
          ...
          <span className="pointer-events-none">{t.mobileStickyCta.book}</span>
        </Link>
        <a
          href={waHref}
          target="_blank"
        ...
        >
          ...
          <span className="pointer-events-none">{t.mobileStickyCta.whatsapp}</span>
        </a>
      </div>
    </motion.div>
  );
