...
const ChiSiamo = () => {
  ...
  // Offset è fondamentale: dice a Framer Motion di iniziare a calcolare da quando
  // la sezione tocca il top dello schermo, fino a quando non finisce.
  const { scrollYProgress } = useScroll({
    target: scrollRef,
    offset: ["start start", "end end"],
  });

  // Movimento da 0% a -66.666% per far scorrere le 3 slide esattamente.
  const x = useTransform(scrollYProgress, [0, 1], ["0%", "-66.666%"]);

  return (
    // Usa overflow-clip invece di overflow-x-hidden per non rompere il position: sticky!
    <div className="bg-[#050505] text-white selection:bg-gold selection:text-black pt-20 overflow-clip font-sans">
      {/* 1. HERO EDITORIALE */}
      ...
      {/* 4. TRUSTEMAQUE - I PILASTRI (Carosello Orizzontale a Scorrimento Verticale) */}
      <section ref={scrollRef} className="relative h-[300vh] bg-[#050505]">
        {/* Il contenitore sticky si fissa in cima e fa da finestra per lo slide */}
        <div className="sticky top-0 h-screen w-full flex items-center overflow-hidden">
          {/* motion.div muove i contenuti a sinistra in base allo scorrimento della pagina */}
          <motion.div style={{ x }} className="flex w-[300vw] h-full items-center">
            {/* Slide 1 */}
            <div className="w-screen h-full flex flex-col items-center justify-center px-4 md:px-24">
      ...
      {/* 7. BEST CAR LINK EROICO ALLA PRENOTAZIONE */}
      <Link
        to="/prenotaora"
